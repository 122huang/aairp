...
type StubFindingPayload = {
  risk_type: string;
  description: string;
  severity: 'HIGH' | 'MEDIUM' | 'LOW';
  suggested_action: LlmSuggestedAction | 'REJECT';
  confidence: number;
...
type StubResponsePayload = {
  prompt_pack_version?: string;
  findings: StubFindingPayload[];
};
...
export function parseOpenRiskStubResponse(content: string): StubResponsePayload {
  const parsed = JSON.parse(content) as StubResponsePayload;
  if (!parsed || !Array.isArray(parsed.findings)) {
    throw new Error('invalid open risk stub response: findings array required');
  }
  return parsed;
}
  ...
  async discover(
    ...
    if (prior.hasBlocker) {
      return {
        ...
        promptPackVersion: this.config.promptPackVersion ?? 'demo-open-risk-1.1.0',
        findings: [],
        skipped: true,
    ...
    const stubPayload = parseOpenRiskStubResponse(completion.content);
    const promptPackVersion =
      stubPayload.prompt_pack_version ?? 'demo-open-risk-1.1.0';
