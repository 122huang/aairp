import { validateEvidenceCorpus } from './evidence-corpus-validator.js';

async function main(): Promise<void> {
  const result = validateEvidenceCorpus();
  console.log('Evidence Corpus Validation');
  ...
  if (result.error_count > 0) {
    ...
    for (const issue of result.issues.filter((item) => item.severity === 'error')) {
      console.log(`  [${issue.code}] ${issue.evidence_id}: ${issue.message}`);
    }
    process.exitCode = 1;
  }
