...
pack_version: demo-playbook-1.5.6
...
severity_hint: HIGH
decision: WARN
guidance: Quantitative or efficacy performance claim detected. Verify substantiation, test conditions, and on-pack specifications before publishing. Oil/energy-reduction percentages (e.g. 70%/75%/80% / 节能高达30%) require verifiable test data on file.
typical_decision: REVIEW
skill_module: Claim Review
...
severity_hint: HIGH
decision: WARN
guidance: 减少用油量/节能等量化宣称（如「减少高达70%/75%/80%用油量」「节能高达30%」）须持有可核验的测试数据（对照方法、机型、测算口径）后方可发布。裸百分比无油/能耗语境时不命中本模式。
typical_decision: REVIEW
skill_module: Claim Review
...
severity_hint: HIGH
decision: WARN
guidance: BPA-free 等材质安全宣示须持有第三方检测报告（如 SGS、Intertek 等）后方可发布；无报告则改为中性材质描述或备查后再用。
typical_decision: REVIEW
skill_module: Evidence Review
...
severity_hint: MEDIUM
decision: WARN
guidance: 类比/感官等同宣称（餐厅级、柴火、石锅、面包房等）须有感官测试或合理限定语支撑，避免暗示必然达到专业或传统烹饪效果。
typical_decision: REVIEW
skill_module: Claim Review
...
severity_hint: MEDIUM
decision: WARN
guidance: 环保/可持续宣称须有材料、能耗或生产工艺证据支撑，避免笼统「守护地球」式表述。
typical_decision: REVIEW
skill_module: Claim Review
